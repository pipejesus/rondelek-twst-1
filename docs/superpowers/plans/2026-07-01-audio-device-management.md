...
- **Deviation applied after final review (commit 9a41ed3):** Task 4's `maintain_audio` `None` arms as written here (`self.playback = None` / `self.capture = None`) tear down a *still-alive* stream when device enumeration transiently returns `None` mid-transition, causing a ~1s audible dropout. The `None` arms now only tear down when the existing stream is not `is_alive()` — a healthy stream is kept and the tick is skipped. This strengthens the resilience the feature exists for; approved by the maintainer over the plan's original unconditional teardown.
